...
<span class="close" onclick="window.closeModalByName('cronModal')">&times;</span>
<h2>⏰ 定时任务管理</h2>
<div style="margin:12px 0;background:#f5f7fa;padding:16px;border-radius:8px;">
    <div style="margin-bottom:12px;">
        ...
        <select id="cronMode" onchange="window.cronModeChange()" style="width:100%;padding:8px 12px;border:1px solid #ddd;border-radius:6px;font-size:14px;background:#fff;">
            <option value="custom">🔧 自定义 (Cron表达式)</option>
            <option value="daily">📅 每天</option>
            <option value="weekly">📅 每周</option>
            <option value="hourly">⏰ 每小时</option>
            <option value="minutes">⏱️ 每N分钟</option>
            <option value="timerange">🔄 时间段循环（几点~几点，每N分钟/每小时）</option>
        </select>
    </div>
    <!-- 每天 -->
    ...
    <!-- 时间段循环 新增模块 -->
<div id="cronTimeRange" style="display:none;margin-bottom:12px;background:#f0f7ff;padding:12px;border-radius:6px;">
    <div style="margin-bottom:10px;">
        <label style="font-weight:500;font-size:13px;color:#555;display:block;margin-bottom:4px;">起始小时</label>
        <select id="rangeStartHour" onchange="window.updateCronSchedule()" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;">
            <option value="0">00</option><option value="1">01</option><option value="2">02</option><option value="3">03</option>
            <option value="4">04</option><option value="5">05</option><option value="6">06</option><option value="7">07</option>
            <option value="8">08</option><option value="9">09</option><option value="10">10</option><option value="11">11</option>
            <option value="12">12</option><option value="13">13</option><option value="14">14</option><option value="15">15</option>
        ...
        <label style="font-weight:500;font-size:13px;color:#555;display:inline-block;margin-bottom:4px;">结束小时</label>
        <select id="rangeEndHour" onchange="window.updateCronSchedule()" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;">
            <option value="0">00</option><option value="1">01</option><option value="2">02</option><option value="3">03</option>
            <option value="4">04</option><option value="5">05</option><option value="6">06</option><option value="7">07</option>
            <option value="8">08</option><option value="9">09</option><option value="10">10</option><option value="11">11</option><option value="12" selected>12</option><option value="13">13</option><option value="14">14</option><option value="15">15</option><option value="16">16</option><option value="17">17</option><option value="18">18</option><option value="19">19</option><option value="20">20</option><option value="21">21</option><option value="22">22</option><option value="23">23</option>
        </select>
    </div>
    <div style="margin-bottom:8px;">
        <label style="font-weight:500;font-size:13px;color:#555;display:block;margin-bottom:4px;">循环间隔</label>
        <select id="rangeIntervalType" onchange="window.updateCronSchedule()">
            <option value="hour">每小时整点执行</option>
            <option value="min">每N分钟执行</option>
        </select>
    </div>
    <div id="rangeMinBox" style="display:none;margin-bottom:6px;">
        <label>每</label>
        <!-- ===== 修改：将下拉框改为数字输入框，支持任意正整数 ===== -->
        <input type="number" id="rangeMinNum" min="1" value="15" onchange="window.updateCronSchedule()" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;">
        <!-- ===== 修改结束 ===== -->
        <span>分钟</span>
    </div>
    <!-- ===== 日、月、周 文本输入框（支持范围/列表） ===== -->
    <div style="margin-top:10px;border-top:1px dashed #ccc;padding-top:10px;">
        <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
            <div>
                <label style="font-weight:500;font-size:13px;color:#555;display:block;margin-bottom:4px;">日 (每月第几天)</label>
                <input type="text" id="rangeDay" value="*" onchange="window.updateCronSchedule()" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;font-family:monospace;" placeholder="* 或 1-31">
            </div>
            <div>
                <label style="font-weight:500;font-size:13px;color:#555;display:block;margin-bottom:4px;">月</label>
                <input type="text" id="rangeMonth" value="*" onchange="window.updateCronSchedule()" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;font-family:monospace;" placeholder="* 或 1-12">
            </div>
            <div>
                <label style="font-weight:500;font-size:13px;color:#555;display:block;margin-bottom:4px;">星期几</label>
                <input type="text" id="rangeWeekday" value="*" onchange="window.updateCronSchedule()" style="width:80px;padding:6px 8px;border:1px solid #ddd;border-radius:4px;font-size:14px;font-family:monospace;" placeholder="* 或 0-6">
            </div>
        </div>
        <div style="font-size:12px;color:#888;margin-top:4px;">💡 支持 <code>*</code>（不限）、范围如 <code>5-12</code>、列表如 <code>1,3,5</code>、步长如 <code>*/2</code> 等标准 cron 语法。</div>
    </div>
    <div style="font-size:12px;color:#888;margin-top:4px;">💡 示例：5~12点，每小时整点，每天 → 0 5-12 * * *</div>
</div>     
    <!-- 自定义 -->
    ...
    <button class="btn-success" onclick="window.cronAdd()" style="width:100%;padding:10px;border:none;border-radius:6px;cursor:pointer;font-weight:500;background:#4caf50;color:#fff;font-size:14px;">➕ 添加任务</button>
</div>
<div style="margin-top:16px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        <label style="font-weight:500;font-size:14px;color:#555;">📋 当前任务列表</label>
        <button class="btn-secondary" onclick="window.cronRefreshList()" style="padding:4px 14px;border:none;border-radius:4px;cursor:pointer;font-size:12px;background:#eceff1;">🔄 刷新</button>
    </div>
...
<div style="margin-top:16px;text-align:right;">
    <button class="btn-secondary" onclick="window.closeModalByName('cronModal')" style="padding:6px 24px;border:none;border-radius:6px;cursor:pointer;font-weight:500;background:#eceff1;">关闭</button>
</div>
</div>
</div>
...
/*
================================================================
⚠️ ⚠️ ⚠️ 窗口创建逻辑与铁律（必读）⚠️ ⚠️ ⚠️
================================================================

【为什么 HTML 注释不能放在文件最前面？】
  如果本文件以 <!-- 注释 --> 开头，app.py 的 loadModal 解析时可能出错，
  导致所有弹窗无法显示。因此，所有说明必须写在 <script> 的 JS 注释中。

【通用弹窗创建流程】
  1. 在 tools/ 下的脚本前10行内添加 # popup: 弹窗ID (例如 # popup: myModal)
  2. 如果需要独立 HTML 文件，再加一行 # html: 文件名.html
     （文件必须放在 /root/scripts/static/ 下）
  3. 在本文件（或独立的 HTML 文件）中添加弹窗的 HTML 结构：
     <div class="modal active" id="myModal" style="display:none;">
       ...
     </div>
  4. 在本文件的 <script> 中（或独立文件的 <script> 中）添加业务函数，
     所有函数必须挂载到 window：
       window.init_myModal = function() { ... };
       window.myAction = function() { ... };
  5. 弹窗右上角的 X 按钮和底部的关闭按钮必须调用 window.closeModalByName('弹窗ID')
  6. 如果使用独立 HTML 文件，该文件中也需要定义 window.closeModalByName （或依赖主文件已加载的该函数）

【函数挂载铁律】
  - 所有需要被 HTML onclick 调用的函数，必须写成 window.xxx = function(){}
  - 初始化函数必须命名为 window.init_弹窗ID
  - 普通辅助函数（如 populateSelect）如果只在弹窗内部使用，可以不挂 window，
    但建议统一挂载以避免作用域问题。

【关闭按钮铁律】
  - 所有弹窗的关闭操作都必须通过 window.closeModalByName 或 window.closeProcModal 等全局关闭函数。
  - 关闭函数应只隐藏弹窗（el.style.display = 'none'），不要删除 DOM，
    否则再次打开时可能出错。
  - 如果需要在关闭时清空容器并重置 modalLoaded（如返回主界面），
    可以保留原有逻辑（container.innerHTML = ''; modalLoaded = false;），
    但必须确保后续打开能重新加载。

【独立 HTML 弹窗的使用】
  - 当脚本头部指定了 # html: 文件名.html 时，面板会加载 /static/ 下的该文件。
  - 独立文件只需包含一个弹窗的 HTML 和对应的 JS，无需包含其他弹窗。
  - 独立文件中的函数也必须挂载到 window。
  - 建议每个独立文件都定义自己的 window.closeModalByName （内容与主文件一致）。

【添加按钮】
  - 在脚本头部添加 # btn: 按钮文字, # group: script/router, # order: 数字,
    # action: runScript:脚本名.py (或 runTool:脚本名.py), # btn-class: 颜色类
  - app.py 会自动扫描并生成按钮，无需手动修改 app.py。

================================================================
*/

// ========== 关闭函数（独立文件必须定义） ==========
window.closeModalByName = function(name) {
...
window.init_cronModal = function() {
    window.loadScriptsForCron();
    window.cronRefreshList();
    // 修复BUG：弹窗初始化自动执行一次面板显隐刷新，首次打开时间段循环分钟框正常显示
    window.cronModeChange();
};
window.loadScriptsForCron = function() {
    fetch('/api/run_tool', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ script: 'cron_manager.py', args: ['--list-scripts'] })
    })
    .then(r => r.json())
    .then(d => {
        ...
        var scripts = JSON.parse(d.output);
        if (scripts && scripts.length) {
            scripts.forEach(function(s) {
                var opt = document.createElement('option');
                opt.value = 'python3 /root/scripts/' + s;
                opt.textContent = s;
                sel.appendChild(opt);
            });
        }
...
window.cronModeChange = function() {
    ...
    // 新增时间段面板控制
    document.getElementById('cronTimeRange').style.display = (mode === 'timerange') ? 'block' : 'none';
    // 间隔类型切换
    var intervalType = document.getElementById('rangeIntervalType').value;
    document.getElementById('rangeMinBox').style.display = (intervalType === 'min') ? 'block' : 'none';
    window.updateCronSchedule();
};
window.updateCronSchedule = function() {
    var mode = document.getElementById('cronMode').value;
    var schedule = '';
    switch (mode) {
        case 'daily':
        ...
        case 'minutes':
            ...
            schedule = '*/' + interval + ' * * * *';
            break;
        // 新增：时间段循环逻辑（日、月、周从文本输入框读取）
        case 'timerange':
            // 兜底二次刷新分钟框显示，防止切换异常
            var intervalType = document.getElementById('rangeIntervalType').value;
            document.getElementById('rangeMinBox').style.display = (intervalType === 'min') ? 'block' : 'none';

            var startH = document.getElementById('rangeStartHour').value;
            var endH = document.getElementById('rangeEndHour').value;
            var intType = document.getElementById('rangeIntervalType').value;
            // 从输入框读取日、月、周
            var day = document.getElementById('rangeDay').value.trim() || '*';
            var month = document.getElementById('rangeMonth').value.trim() || '*';
            var weekday = document.getElementById('rangeWeekday').value.trim() || '*';

            var minutePart = '';
            if(intType === 'hour'){
                minutePart = '0';
            }else{
                var minNum = document.getElementById('rangeMinNum').value.trim() || '15';
                minutePart = '*/' + minNum;
            }
            // 组合：分 时 日 月 周
            schedule = minutePart + ' ' + startH + '-' + endH + ' ' + day + ' ' + month + ' ' + weekday;
            break;
        case 'custom':
        default:
            return;
...
window.cronRefreshList = function() {
        ...
        try {
            ...
            jobs.forEach(function(job) {
                ...
                html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border-bottom:1px solid #f0f0f0;">' +
                    ...
                    '<button onclick="window.cronDelete(\'' + escaped + '\')" style="background:#f44336;color:#fff;border:none;border-radius:4px;padding:2px 12px;cursor:pointer;font-size:12px;flex-shrink:0;">🗑</button>' +
                    '</div>';
            });
...
window.cronDelete = function(fullLine) {
    ...
    .then(r => r.json()).then(d => { alert(d.output || '执行完成'); window.cronRefreshList(); })
    .catch(e => { alert('删除失败: ' + e.message); });
};
window.cronAdd = function() {
    // 添加前强制刷新 cron 表达式，确保使用最新设置
    window.updateCronSchedule();
